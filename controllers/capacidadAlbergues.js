...
const getMethod = (req = request, res = response) => {
    ...
    pool.query('CALL pa_SelectCapacidadAlbergue(?)', [id], (error, results) => {
        if (error) {
            ...
            return res.status(500).json({ success: false, error: 'Error al obtener capacidad' });
        }
    ...
    pool.query(
        ...
        (error, results) => {
            if (error) {
                ...
                return res.status(500).json({ success: false, error: 'Error al crear registro' });
            }
    ...
    pool.query(
        ...
        (error, results) => {
            if (error) {
                ...
                return res.status(500).json({ success: false, error: 'Error al actualizar registro' });
            }
...
const deleteMethod = (req = request, res = response) => {
    ...
    pool.query('CALL pa_DeleteCapacidadAlbergue(?)', [id], (error, results) => {
        if (error) {
            ...
            return res.status(500).json({ success: false, error: 'Error al eliminar registro' });
        }
